...
export default function GroupRoom() {
  ...
  useEffect(() => {
    ...
    // join the room for chat + video sync
    s.emit('join-room', { roomId, username, isHost });
    s.on('participants', cnt => setParticipants(cnt));
    s.on('chat-message', msg => {
      // host sees everyone; guests skip local-echo
      if (!isHost && msg.author === username) return;
      setMessages(m => [...m, msg]);
    });
    s.on('play',  data => {
    ...
    // helper that binds a new simple-peer with our localStream
    function createPeer(userId, initiator) {
      ...
      peer.on('signal', sig => s.emit('signal', { to: userId, signal: sig }));
  ...
  const handlePlay = e => socket.emit('play',  { roomId, currentTime: e.target.getCurrentTime() });
  const handlePause = e => socket.emit('pause', { roomId, currentTime: e.target.getCurrentTime() });
  const handleSeek = e => socket.emit('seek',  { roomId, currentTime: e.target.getCurrentTime() });
    ...
    >
      ...
      <div className="flex flex-col md:w-2/3 flex-1">
        <header className="p-4 flex justify-between items-center 
          ...
          <button
            className="px-2 py-1 border rounded border-white/50
      ...
      {/* Chat sidebar */}
      <div className="w-full md:w-1/3 flex flex-col border-l border-gray-700
                       bg-gray-800">
        <div className="flex-1 p-4 overflow-y-auto space-y-3
                      scrollbar-track-gray-700 scrollbar-thumb-indigo-500 scrollbar-thin">
          {messages.map((m, i) => (
            <motion.div
              key={i}
        ...
        <div className="p-4 border-t border-gray-700 flex space-x-2">
          <input
            className="flex-1 border-2 border-gray-600 bg-gray-900 p-2 rounded
      ...
      {/* 3) Mute/Unmute button bottom‑center (visible to everyone) */}
      <motion.button
        onClick={toggleMic}
        whileHover={{ scale: 1.05 }}
        whileTap={{ scale: 0.95 }}
      ...
      {/* Render each peer's incoming audio (off‑screen but still audible) */}
